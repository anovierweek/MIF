...
class MIF():
    ...
    def get_cmd_loss(self,):
        ...
        if not self.args.use_cmd_sim:
            return 0.0
        loss = torch.mean(abs(F.cosine_similarity(self.model.Model.utt_shared_t, self.model.Model.utt_shared_v, dim=0)), dim=0, keepdims=False)
        loss += torch.mean(abs(F.cosine_similarity(self.model.Model.utt_shared_t, self.model.Model.utt_shared_a, dim=0)), dim=0, keepdims=False)
        loss += torch.mean(abs(F.cosine_similarity(self.model.Model.utt_shared_a, self.model.Model.utt_shared_v, dim=0)), dim=0, keepdims=False)
        loss = loss/3.0
        return loss
